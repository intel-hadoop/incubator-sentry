...
public class TestSentryGenericPolicyProcessor {
  ...
  private void testOperation(String requestUser, Status validateStatus) throws Exception {
    ...
    dropPrivRequest.setPrivilege(new TSentryPrivilege("test", "test", new ArrayList<TAuthorizable>(), "test", "test"));
  ...
  public void testAdminOperation() throws Exception {
    when(mockStore.createSentryRole(anyString(), anyString()))
        .thenReturn(new CommitContext(SERVER_UUID, SEQ_ID));

    when(mockStore.dropSentryRole(anyString(), anyString()))
        .thenReturn(new CommitContext(SERVER_UUID, SEQ_ID + 1));

    when(mockStore.alterSentryRoleAddGroups(anyString(), anySetOf(String.class),anyString()))
        .thenReturn(new CommitContext(SERVER_UUID, SEQ_ID + 2));

    when(mockStore.alterSentryRoleDeleteGroups(anyString(),anySetOf(String.class), anyString()))
        .thenReturn(new CommitContext(SERVER_UUID, SEQ_ID + 3));

    when(mockStore.dropSentryPrivilege(any(PrivilegeObject.class), anyString()))
        .thenReturn(new CommitContext(SERVER_UUID, SEQ_ID + 4));

    when(mockStore.renameSentryPrivilege(anyString(), anyString(), anyListOf(Authorizable.class),
  ...
  public void testGrantAndRevokePrivilege() throws Exception {
    when(mockStore.alterSentryRoleGrantPrivilege(anyString(), any(PrivilegeObject.class), anyString()))
    .thenReturn(new CommitContext(SERVER_UUID, SEQ_ID + 6));

    when(mockStore.alterSentryRoleRevokePrivilege(anyString(),any(PrivilegeObject.class), anyString()))
    .thenReturn(new CommitContext(SERVER_UUID, SEQ_ID + 7));
    setup();

    TSentryPrivilege tprivilege = new TSentryPrivilege("test", "test", new ArrayList<TAuthorizable>(), "test", "test");
  ...
  public void testGetRolesAndPrivileges() throws Exception {
    ...
    when(mockStore.getRolesByGroupNames(anySetOf(String.class)))
    ...
    when(mockStore.getGroupsByRoleNames(anySetOf(String.class)))
  ...
  public void testConfigCannotCreateSentryStore() throws Exception {
    Configuration conf = new Configuration();
    conf.set(PolicyStoreConstants.SENTRY_POLICY_STORE,"junk");
