...
public class TestSentryGenericServiceIntegration extends SentryServiceIntegrationBase {
  ...
  public void beforeSetup() throws Exception {
    conf.set(
        ServerConfig.PROCESSOR_FACTORIES,
        "org.apache.sentry.provider.db.genericModel.service.thrift.SentryGenericPolicyProcessorFactory");
  }
  ...
  public void testGetPrivilegeByHierarchy() throws Exception {
    ...
    assertEquals(2, client.listPrivilegesByRoleName(requestorUserName, roleName,SOLR, "service1").size());
    ...
    ActiveRoleSet roleSet = new ActiveRoleSet(Sets.newHashSet(roleName));

    assertEquals(1, client.listPrivilegesForProvider(SOLR, "service1", roleSet,
        null, Arrays.asList(new Collection("c1"))).size());

    assertEquals(1, client.listPrivilegesForProvider(SOLR, "service1", roleSet,
        null, Arrays.asList(new Collection("c2"))).size());

    assertEquals(1, client.listPrivilegesForProvider(SOLR, "service1", roleSet,
        null, Arrays.asList(new Collection("c1"), new Field("f1"))).size());

    assertEquals(1, client.listPrivilegesForProvider(SOLR, "service1", roleSet,
        null, Arrays.asList(new Collection("c2"), new Field("f2"))).size());
  }
