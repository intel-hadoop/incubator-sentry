...
public class PoolClientInvocationHandler<T extends SentryPolicyServiceBaseClient> extends SentryClientInvocationHandler {
  ...
  public PoolClientInvocationHandler(Configuration conf, Class<?> factoryClass) throws Exception {
    this.conf = conf;
    readConfiguration();
    poolFactory = new SentryServiceClientPoolFactory<T>(conf, factoryClass);
    pool = new GenericObjectPool<T>(poolFactory, poolConfig, new AbandonedConfig());
  }
  ...
  private Object invokeFromPool(Object proxy, Method method, Object[] args) throws Exception {
    ...
    try {
      ...
      // Get the wrapped exception, check if TTransportException is wrapped. TTransportException
      // means there has connection problem with the pool.
      Throwable throwable1 = e.getCause();
      Throwable throwable2 = null;
      if (throwable1 != null) {
        // If there has connection problem, eg, invalid connection if the service restarted,
        // throwable2 instanceof TTransportException = true.
        throwable2 = throwable1.getCause();
      }
      // If the exception is caused by connection problem, destroy the instance and remove it from
      // the commons-pool. Throw the TTransportException for reconnect.
      if (throwable2 != null && throwable2 instanceof TTransportException) {
        pool.invalidateObject(client);
        throw new TTransportException(throwable2);
      } else if (throwable1 != null && throwable1 instanceof SentryUserException) {
